...
1. 內部 3–5 人：各完成一趟「行前 → 旅途中 → 回國後」演練，並至少一人完成 `DEVICE_QA.md` 的 travel order 刪除／同檔重傳 smoke test。
2. 觀察 48 小時：上傳失敗率、OCR 待確認率、pending storage cleanup、匯出差額皆為可接受範圍。
3. 每個觀察時段由管理者開啟「系統管理 → 系統健康」：記錄 pending 數量、最舊等待時間與最高 attempts。正常情況應回落至 0；若短暫出現 pending，可先執行「重試待清理附件」。
4. 擴至一個部門；保留舊流程作為短期備援。
...
出現任一條件即停止擴大，保留資料並執行回復或修復；不得用手動刪資料掩蓋健康指標。
