...
export default function Home(){
 ...
 const appHeader=<header className="topbar"><div className="topbar-inner">
  ...
  <nav aria-label="主要流程"><button className={stage==="create"?"active":""} onClick={goTrips}><i>1</i>我的出差</button><button disabled={!activeTrip} className={stage==="itinerary"?"active":""} onClick={()=>activeTrip&&openTrip(activeTrip,"itinerary")}><i>2</i>行程</button><button disabled={!activeTrip} className={stage==="expense"?"active":""} onClick={()=>activeTrip&&openTrip(activeTrip,"expense")}><i>3</i>我的報支</button></nav>
