...
const resolveTripStage=(value:string|null):TripStage=>value==="expense"?"expense":"itinerary";
...
export default function Home(){
 ...
 useEffect(()=>{let active=true;const restore=async()=>{const params=new URLSearchParams(window.location.search),tripId=params.get("trip"),target=params.get("stage");if(!tripId){if(active){setStage("create");setActiveTrip(null)}return}const response=await fetch("/api/trips");if(!response.ok)return;const trip=(await response.json()).trips?.find((item:ActiveTrip)=>item.id===tripId);if(active&&trip){const nextStage=resolveTripStage(target);setActiveTrip({id:trip.id,name:trip.name});setStage(nextStage);if(target!==nextStage){const url=new URL(window.location.href);url.searchParams.set("stage",nextStage);window.history.replaceState(null,"",`${url.pathname}${url.search}`)}}};restore().catch(()=>null);window.addEventListener("popstate",restore);return()=>{active=false;window.removeEventListener("popstate",restore)}},[]);
 ...
 const openTrip=(trip:ActiveTrip,target:TripStage="itinerary")=>{setActiveTrip(trip);setStage(target);const url=new URL(window.location.href);url.searchParams.set("trip",trip.id);url.searchParams.set("stage",target);window.history.pushState(null,"",`${url.pathname}${url.search}`)};
