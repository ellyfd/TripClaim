...
const travelIntakeMessage="機票／住宿請從「行前準備 → 我的行前資料」上傳，才能同步行程與報支。";
...
export default function ExpenseWizardLive({tripId}:{tripId:string}){
 ...
 return <main className="expense-workbench-page" id="my-expense">
  <header className="expense-workbench-head"><div><span>只顯示本人資料</span><h1>我的報支</h1><p>費用、文件、缺件與匯出都集中在這裡；機票與住宿則從行前準備同步進來。</p></div><button className="expense-drawer-trigger" onClick={()=>setShowDesktopTools(true)}>工具與文件</button></header>
  <>
