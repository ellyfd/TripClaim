...
export const CLAIM_TYPE_CODES=Object.fromEntries(DEFAULT_CLAIM_TYPES.map((label,index)=>[label,`EXP-${String(index+1).padStart(2,"0")}`])) as Record<string,string>;
...
export const managedClaimTypeCode=(value:string|undefined|null)=>value&&CLAIM_TYPE_CODES[value]||null;
export const resolveManagedDestination=(countryCode:string|undefined,countryName:string|undefined,cityName:string|undefined)=>{
 if(!countryCode||!countryName||!cityName)return null;
 const country=DEFAULT_COUNTRIES.find(x=>x.endsWith(` ${countryCode.toUpperCase()}`));
 if(!country||!country.startsWith(countryName))return null;
 const cityIndex=DEFAULT_DESTINATIONS[country]?.indexOf(cityName)??-1;
 return cityIndex<0?null:{countryCode:countryCode.toUpperCase(),countryName:country.slice(0,-4),cityName,cityCode:`${countryCode.toUpperCase()}-${String(cityIndex+1).padStart(3,"0")}`};
};
export const isManagedDestination=(countryCode:string|undefined,countryName:string|undefined,cityName:string|undefined)=>{
 return Boolean(resolveManagedDestination(countryCode,countryName,cityName));
};
