...
type Summary={trip?:{name:string;startsOn:string;endsOn:string};destinations?:Array<{cityName:string;countryName:string}>;members?:Array<{userEmail:string}>;agenda?:Array<{id:string;title:string;startsAt:string;type:string}>;bookings?:Array<{id:string;ownerEmail:string;kind:"flight"|"stay";documentId?:string|null;bookedAt:string}>};
...
const browserLocalNowKey=()=>{const now=new Date();return `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}T${pad(now.getHours())}:${pad(now.getMinutes())}`};
const sourceOrderKey=(booking:NonNullable<Summary["bookings"]>[number])=>booking.documentId?`document:${booking.documentId}`:`manual:${booking.ownerEmail}:${booking.kind}:${booking.bookedAt}`;
...
export default function TripOverview({tripId,onNavigate}:{tripId:string;onNavigate:(stage:WorkspaceStage)=>void}){
 ...
 const agendaCount=summary?.agenda?.length??0,sourceOrderCount=new Set((summary?.bookings??[]).map(sourceOrderKey)).size;
 const unresolved=missing?.requirements?.filter(item=>item.status!=="complete").length??0;
 const upcomingAgenda=summary?.agenda?.find(item=>item.startsAt>=browserLocalNowKey())??null;
 ...
 return <main className="trip-overview-page">
  ...
  <section className="overview-status-grid" aria-label="出差工作區狀態">
   ...
   <article className="overview-card itinerary"><span>行程</span><strong>{agendaCount}</strong><p>{agendaCount?`已有 ${agendaCount} 筆共同行程${sourceOrderCount?`・來自 ${sourceOrderCount} 張機票／住宿訂單`:""}`:"還沒有共同行程"}</p><button onClick={()=>onNavigate("itinerary")}>{agendaCount?"查看／編輯行程":"開始安排行程"}</button></article>
  ...
  <section className="overview-detail-grid">
   <article className="panel overview-next">{upcomingAgenda?<><span>下一個行程</span><h2>{upcomingAgenda.title}</h2><p>{upcomingAgenda.startsAt.replace("T"," ")}・{upcomingAgenda.type}</p><button onClick={()=>onNavigate("itinerary")}>打開行程</button></>:agendaCount?<><span>行程狀態</span><h2>目前沒有後續安排</h2><p>這趟出差已有 {agendaCount} 筆行程；可打開行程查看過去安排或新增活動。</p><button onClick={()=>onNavigate("itinerary")}>查看行程</button></>:<><span>行程狀態</span><h2>尚未安排行程</h2><p>先把共同行程排進日曆，同行者就能看到同一份最新版本。</p><button onClick={()=>onNavigate("itinerary")}>新增第一筆行程</button></>}</article>
