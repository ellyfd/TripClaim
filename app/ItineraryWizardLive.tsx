...
export default function ItineraryWizardLive({tripId}:{tripId:string}){
 ...
 const save=async(row:Agenda)=>{if(isSyncedTravel(row)){setEditing(null);setStatus("機票與住宿由原始訂單同步，請從訂單資料修改");return}setStatus("正在保存…");if(row.version===0){const r=await fetch(`/api/trips/${tripId}/agenda`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(row)});if(r.ok){const x=await r.json();setRows(v=>v.map(y=>y.id===row.id?{...row,id:x.id,version:x.version}:y));setEditing(null);setStatus("已同步給所有同行者")}else setStatus("保存失敗，請重試");return}const r=await fetch(`/api/trips/${tripId}/agenda/${row.id}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify(row)});if(r.status===409){const x=await r.json().catch(()=>null);setStatus(x?.error==="managed_travel_item"?"機票與住宿由原始訂單同步，請從訂單資料修改":"其他同行者剛修改這筆資料，請重新載入後再確認");return}if(r.ok){const x=await r.json();setRows(v=>v.map(y=>y.id===row.id?{...row,version:x.version}:y));setEditing(null);setStatus("已同步給所有同行者")}else setStatus("保存失敗，請重試")};
 const remove=async(row:Agenda)=>{const travel=isSyncedTravel(row);if(!window.confirm(travel?`確定永久刪除「${row.title}」所屬的整張機票／住宿訂單？同張訂單的所有航段、附件與本人待報支會一起刪除，且無法復原。`:`確定刪除「${row.title}」？`))return;if(row.version===0){setRows(v=>v.filter(x=>x.id!==row.id));return}const r=await fetch(`/api/trips/${tripId}/agenda/${row.id}`,{method:"DELETE"});const result=await r.json().catch(()=>null);if(r.ok){setRows(v=>v.filter(x=>x.id!==row.id));setEditing(current=>current===row.id?null:current);setStatus(result?.travelOrder?`已永久刪除整張訂單（${result.bookingsDeleted??1} 段）`:"已刪除並保留復原紀錄");window.dispatchEvent(new Event("tripclaim:data-changed"));if(result?.travelOrder)await load(tripId)}else if(r.status===403&&result?.error==="travel_booking_owner_only")setStatus("這是同行者的機票／住宿，只有訂單本人可以刪除");else setStatus(result?.message||"刪除失敗，請重試")};
