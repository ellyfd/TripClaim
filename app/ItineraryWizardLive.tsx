...
export default function ItineraryWizardLive({tripId}:{tripId:string}){
 ...
 return <main className="wizard-page itinerary-wizard itinerary-workspace-page shared-itinerary-page">
  <section className="wizard-stage itinerary-single">
   <div className="step-heading"><span>Shared workspace</span><h1>行程</h1><p>這一頁只處理所有同行者共用的日曆與 shared bookings；個人機票、住宿、待辦與補休已移到「行前準備」。</p><button className="add-agenda" onClick={add} disabled={!tripId||!dates.length}>＋ 新增活動</button></div>
   ...
   <div className="agenda-workspace shared-only"><div className="agenda-workspace-main">
    ...
    <section className="shared-booking-overview" aria-label="同行者 shared bookings"><BookingPanel tripId={tripId}/></section>
   </div></div>
  </section>
 </main>;
}
