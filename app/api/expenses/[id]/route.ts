...
export async function PATCH(request:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 const [sourceBooking]=before.sourceBookingId?await db.select().from(travelBookings).where(and(eq(travelBookings.id,before.sourceBookingId),eq(travelBookings.tripId,before.tripId),eq(travelBookings.ownerEmail,user.email),isNull(travelBookings.deletedAt))).limit(1):[];
 if(before.sourceBookingId&&!sourceBooking)return NextResponse.json({error:"source_booking_missing",message:"這筆 travel 費用的來源訂單已不存在，請重新整理行程資料"},{status:409});
 const sourceManaged=Boolean(sourceBooking),sourceCategory=sourceBooking?(sourceBooking.kind==="flight"?"機票(自行刷卡)":"住宿"):input.category,sourceExpenseDate=sourceBooking?sourceBooking.startAt.slice(0,10):input.expenseDate,sourceMerchant=sourceBooking?sourceBooking.title:input.merchant;
 const originalCurrency=(sourceBooking?.currency||input.originalCurrency||input.currency||before.originalCurrency||before.currency).trim().toUpperCase(),master=validateExpenseMaster({claimType:sourceCategory,originalCurrency,reportingCurrency:input.reportingCurrency||before.reportingCurrency}),originalAmountMinor=sourceBooking?.amountMinor??input.originalAmountMinor??input.amountMinor??before.originalAmountMinor??before.amountMinor,decision=master.currencyDecision,reportingCurrency=master.reportingCurrency,reportingAmountMinor=input.reportingAmountMinor??input.claimedTwdMinor??before.reportingAmountMinor??(reportingCurrency===originalCurrency?originalAmountMinor:undefined);
 if(!sourceExpenseDate||!master.valid||!sourceMerchant||typeof originalAmountMinor!=="number"||originalAmountMinor<0||typeof reportingAmountMinor!=="number"||reportingAmountMinor<0||!Number.isInteger(input.receiptCount)||input.receiptCount<1||!input.costCenter)return NextResponse.json({error:"invalid_master_data",issues:master.issues},{status:400});
 ...
 const next={expenseDate:sourceExpenseDate,category:sourceCategory,categoryCode:master.claimTypeCode,merchant:sourceMerchant,originalCurrency,originalAmountMinor,reportingCurrency,reportingAmountMinor,currency:reportingCurrency,amountMinor:reportingAmountMinor,cardLast4:input.cardLast4||null,receiptCount:input.receiptCount,remark:input.remark?.trim()||null,costCenter:input.costCenter.trim(),exchangeRate:input.exchangeRate?.trim()||null,exchangeRateSource:input.exchangeRateSource?.trim()||null,currencyDecisionReason:decision.reason,claimedTwdMinor:reportingCurrency==="TWD"?reportingAmountMinor:null,updatedAt:new Date().toISOString()};
 await db.update(personalExpenses).set(next).where(and(eq(personalExpenses.id,id),eq(personalExpenses.ownerEmail,user.email)));await recordAudit({tripId:before.tripId,actorEmail:user.email,entityType:"personal_expense",entityId:id,action:sourceManaged?"update_reporting_fields":"update",before,after:next});return NextResponse.json({saved:true,sourceManaged});
}
...
export async function DELETE(_request:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 if(linkedBookings.length||isTravelDocument){
  ...
  await recordAudit({tripId:before.tripId,actorEmail:user.email,entityType:"personal_expense",entityId:id,action:"hard_delete_order_graph",before,after:{bookingIds:deleted.bookingIds,documentId:deleted.documentId,documentIds:deleted.documentIds,duplicateDocumentsDeleted:deleted.duplicateDocumentsDeleted,objectDeleted:deleted.objectDeleted,objectDeleteFailures:deleted.objectDeleteFailures}});
  return NextResponse.json({deleted:true,permanent:true,documentDeleted:Boolean(deleted.documentId),documentsDeleted:deleted.documentIds.length,duplicateDocumentsDeleted:deleted.duplicateDocumentsDeleted,bookingsDeleted:deleted.bookingIds.length,objectDeleted:deleted.objectDeleted,objectDeleteFailures:deleted.objectDeleteFailures});
 }
