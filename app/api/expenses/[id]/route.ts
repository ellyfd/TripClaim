...
import {decideReportingCurrency,isManagedClaimType,managedClaimTypeCode,MANAGED_CURRENCY_CODES} from "../../../managed-config";
export async function PATCH(request:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 const originalCurrency=(input.originalCurrency||input.currency||before.originalCurrency||before.currency).trim().toUpperCase(),originalAmountMinor=input.originalAmountMinor??input.amountMinor??before.originalAmountMinor??before.amountMinor,decision=decideReportingCurrency(originalCurrency),reportingCurrency=(input.reportingCurrency||before.reportingCurrency||decision.reportingCurrency).trim().toUpperCase(),reportingAmountMinor=input.reportingAmountMinor??input.claimedTwdMinor??before.reportingAmountMinor??(reportingCurrency===originalCurrency?originalAmountMinor:undefined);
 if(!input.expenseDate||!isManagedClaimType(input.category)||!input.merchant||typeof originalAmountMinor!=="number"||originalAmountMinor<0||!MANAGED_CURRENCY_CODES.has(reportingCurrency)||typeof reportingAmountMinor!=="number"||reportingAmountMinor<0||!Number.isInteger(input.receiptCount)||input.receiptCount<1||!input.costCenter)return NextResponse.json({error:"invalid_master_data"},{status:400});
 ...
 const next={expenseDate:input.expenseDate,category:input.category,categoryCode:managedClaimTypeCode(input.category),merchant:input.merchant,originalCurrency,originalAmountMinor,reportingCurrency,reportingAmountMinor,currency:reportingCurrency,amountMinor:reportingAmountMinor,cardLast4:input.cardLast4||null,receiptCount:input.receiptCount,remark:input.remark?.trim()||null,costCenter:input.costCenter.trim(),exchangeRate:input.exchangeRate?.trim()||null,exchangeRateSource:input.exchangeRateSource?.trim()||null,currencyDecisionReason:decision.reason,claimedTwdMinor:reportingCurrency==="TWD"?reportingAmountMinor:null,updatedAt:new Date().toISOString()};
