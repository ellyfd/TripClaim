...
import {MASTER_DATA_VERSION,resolveManagedDestination} from "../../managed-config";
...
export async function POST(request:NextRequest){
 ...
 const resolved=(input.destinations??[]).map(x=>({...x,master:resolveManagedDestination(x.countryCode,x.countryName,x.cityName)}));if(!resolved.length||resolved.some(x=>!x.master))return NextResponse.json({error:"invalid_destination_master_data"},{status:400});
 const db=await ensureUser(user),now=new Date().toISOString(),id=crypto.randomUUID();await db.batch([db.insert(trips).values({id,name:input.name.trim(),purpose:input.purpose?.trim(),startsOn:input.startsOn,endsOn:input.endsOn,createdByEmail:user.email,status:"draft",masterDataVersion:MASTER_DATA_VERSION,createdAt:now,updatedAt:now}),db.insert(tripMembers).values({id:crypto.randomUUID(),tripId:id,userEmail:user.email,role:"creator",joinedAt:now}),...resolved.map((x,i)=>db.insert(tripDestinations).values({id:crypto.randomUUID(),tripId:id,countryCode:x.master!.countryCode,countryName:x.master!.countryName,cityCode:x.master!.cityCode,cityName:x.master!.cityName,timezone:x.timezone,sequence:i}))]);await recordAudit({tripId:id,actorEmail:user.email,entityType:"trip",entityId:id,action:"create",after:{...input,masterDataVersion:MASTER_DATA_VERSION,destinations:resolved.map(x=>x.master)}});return NextResponse.json({id,saved:true,masterDataVersion:MASTER_DATA_VERSION},{status:201});
}
