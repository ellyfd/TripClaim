...
export async function POST(request:NextRequest){
 ...
 const resolved=(input.destinations??[]).map(x=>({...x,master:validateDestinationMaster(x).destination}));if(!resolved.length||resolved.some(x=>!x.master))return NextResponse.json({error:"invalid_destination_master_data"},{status:400});
