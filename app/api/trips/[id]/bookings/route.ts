...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 if(createExpense){const category=input.kind==="flight"?"機票(自行刷卡)":"住宿";writes.push(db.insert(personalExpenses).values({id:expenseId,ownerEmail:user.email,tripId:id,sourceDocumentId:input.documentId,sourceBookingId:bookingId,category,categoryCode:managedClaimTypeCode(category),merchant:booking.title,expenseDate:input.startAt.slice(0,10),originalAmountMinor:booking.amountMinor,originalCurrency,reportingAmountMinor:decision.requiresTwd?null:booking.amountMinor,reportingCurrency:decision.reportingCurrency,currencyDecisionReason:decision.reason,amountMinor:decision.requiresTwd?0:booking.amountMinor,currency:decision.reportingCurrency,status:"review",masterDataVersion:MASTER_DATA_VERSION,createdAt:now,updatedAt:now}));}
