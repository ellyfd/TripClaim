...
import {reportingCurrency} from "../../../../../db/currency";
...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 if(createExpense)writes.push(db.insert(personalExpenses).values({id:expenseId,ownerEmail:user.email,tripId:id,sourceDocumentId:input.documentId,sourceBookingId:bookingId,category:input.kind==="flight"?"機票(自行刷卡)":"住宿",merchant:booking.title,expenseDate:input.startAt.slice(0,10),amountMinor:booking.amountMinor,currency:booking.currency,status:"review",createdAt:now,updatedAt:now}));
