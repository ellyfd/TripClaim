...
import {resolveManagedDestination} from "../../../managed-config";
...
export async function PATCH(request:NextRequest,{params}:Context){
 ...
 const resolved=(input.destinations??[]).map(x=>resolveManagedDestination(x.countryCode,x.countryName,x.cityName));if(!resolved.length||resolved.some(x=>!x))return NextResponse.json({error:"invalid_destination_master_data"},{status:400});
