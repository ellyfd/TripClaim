...
export async function PATCH(request:NextRequest,{params}:Context){
 ...
 const resolved=(input.destinations??[]).map(x=>validateDestinationMaster(x).destination);if(!resolved.length||resolved.some(x=>!x))return NextResponse.json({error:"invalid_destination_master_data"},{status:400});
