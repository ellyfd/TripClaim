...
import {validateCompanyDestination} from "../../../../db/company-master";
...
export async function PATCH(request:NextRequest,{params}:Context){
 ...
 const resolved=await Promise.all((input.destinations??[]).map(x=>validateCompanyDestination(x)));if(!resolved.length||resolved.some(x=>!x.destination))return NextResponse.json({error:"invalid_destination_master_data"},{status:400});
