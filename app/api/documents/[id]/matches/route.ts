...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const expenses=await db.select().from(personalExpenses).where(and(eq(personalExpenses.tripId,document.tripId),eq(personalExpenses.ownerEmail,user.email),isNull(personalExpenses.deletedAt))),needle=words(document.merchant);
