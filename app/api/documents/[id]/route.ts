...
export async function PATCH(request:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 if(!isManagedClaimType(input.claimType))return NextResponse.json({error:"invalid_claim_type"},{status:400});
 ...
 if(input.status==="ready"&&input.expenseDate&&input.merchant&&typeof reportingAmountMinor==="number"){
