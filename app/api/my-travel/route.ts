...
export async function POST(request: NextRequest) {
  ...
  if (input.createExpense !== false && input.amountMinor && input.currency) { const originalCurrency=input.currency.toUpperCase(),decision=decideReportingCurrency(originalCurrency);await db.insert(personalExpenses).values({ id:crypto.randomUUID(), ownerEmail:user.email, tripId:TRIP_ID, sourceDocumentId:id, category:input.kind==="flight"?"機票(自行刷卡)":"住宿", merchant:input.title, expenseDate:input.startAt.slice(0,10), originalAmountMinor:input.amountMinor, originalCurrency, reportingAmountMinor:decision.requiresTwd?null:input.amountMinor, reportingCurrency:decision.reportingCurrency, currencyDecisionReason:decision.reason, amountMinor:decision.requiresTwd?0:input.amountMinor, currency:decision.reportingCurrency, cardLast4:input.cardLast4, status:"review", masterDataVersion:MASTER_DATA_VERSION, createdAt:now, updatedAt:now });}
