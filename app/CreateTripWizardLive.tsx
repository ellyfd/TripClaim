"use client";
import {DateRangeIcon,PlusIcon,PencilIcon,TrashIcon,CheckIcon} from "./icons";
...
type MasterDestination={countryCode:string;countryName:string;cityCode:string;cityName:string};
type MasterCatalog={version:string;destinations:MasterDestination[]};
...
export default function CreateTripWizardLive({onCreate}:{onCreate:(trip:OpenedTrip)=>void}){
 const [destinationOptions,setDestinationOptions]=useState<Record<string,string[]>>({}),[masterVersion,setMasterVersion]=useState("");
 const countries=useMemo(()=>Object.keys(destinationOptions).sort((a,b)=>(a.match(/[A-Z]{3}$/)?.[0]??a).localeCompare(b.match(/[A-Z]{3}$/)?.[0]??b,"en")),[destinationOptions]);
 ...
 useEffect(()=>{Promise.all([fetch("/api/drafts?flow=create").then(r=>r.ok?r.json():null),fetch("/api/trips").then(r=>r.ok?r.json():null),fetch("/api/master-data").then(r=>r.ok?r.json():null)]).then(([drafts,tripData,masterData]:[any,any,MasterCatalog|null])=>{if(masterData?.destinations?.length){const options:Record<string,string[]>={};for(const item of masterData.destinations){const key=`${item.countryName} ${item.countryCode}`;(options[key]??=[]).push(item.cityName)}setDestinationOptions(options);setMasterVersion(masterData.version)}else setErrorMsg("公司國家／城市主檔載入失敗；建立或修改出差前請重新整理。");if(drafts?.draft){const saved={...initial,...drafts.draft.payload};setStep(Math.min(drafts.draft.step,3));setForm(saved);setNameEdited(Boolean(saved.name));setHasCreateDraft(true)}else setHasCreateDraft(false);setTrips(tripData?.trips??[]);setState("ready")}).catch(()=>{setErrorMsg("公司主檔或出差資料載入失敗，請重新整理。");setState("ready")})},[]);
 ...
 const effectiveName=nameEdited?form.name:autoName,masterReady=Boolean(masterVersion&&countries.length);
 const changeStop=(i:number,key:keyof Stop,value:string)=>setForm(v=>({...v,stops:v.stops.map((s,n)=>n===i?{...s,[key]:value,...(key==="country"?{city:value?(destinationOptions[value]?.[0]??""):""}:{})}:s)}));
 ...
 const openTrip=(trip:Trip)=>onCreate({id:trip.id,name:trip.name});
 const startNew=()=>{if(!masterReady){setErrorMsg("公司國家／城市主檔尚未載入，請重新整理後再建立出差。");return}if(!hasCreateDraft){setForm(initial);setNameEdited(false);setEmail("");setStep(1)}setEditingId("");setErrorMsg("");setState("ready");setMode("create")};
 const editTrip=(trip:Trip)=>{if(!masterReady){setErrorMsg("公司國家／城市主檔尚未載入，請重新整理後再修改出差。");return}setForm({name:trip.name,purpose:trip.purpose??trip.name,startsOn:trip.startsOn,endsOn:trip.endsOn,stops:trip.destinations.length?trip.destinations.map(x=>({country:`${x.countryName} ${x.countryCode}`,city:x.cityName})):[{country:"",city:""}],emails:[]});setNameEdited(true);setEditingId(trip.id);setStep(1);setErrorMsg("");setState("ready");setMode("edit")};
 ...
 const firstStepReady=Boolean(masterReady&&effectiveName&&form.startsOn&&form.endsOn&&form.endsOn>=form.startsOn&&!form.stops.some(s=>!s.country||!s.city));
 const finish=async()=>{
  if(!masterReady){setState("error");setErrorMsg("公司國家／城市主檔尚未載入，請重新整理後再保存。");setStep(1);return}
 ...
 if(mode==="list")return <section className="my-trips-page">
  <header className="my-trips-head"><div><span>我的出差</span><h1>全部出差</h1><p>選一趟出差就直接進行程；機票、住宿、待辦與行事曆都在同一頁，報支另外處理。</p></div><div><strong>{trips.length}</strong><small>趟出差</small><button disabled={!masterReady} onClick={startNew}>{hasCreateDraft?"繼續建立草稿":<><PlusIcon/> 建立新出差</>}</button></div></header>
  {errorMsg&&<p className="form-error" role="status">{errorMsg}</p>}
  {state==="loading"?<div className="trips-empty panel">正在載入我的出差…</div>:trips.length===0?<div className="trips-empty panel"><h2>還沒有出差</h2><p>建立第一趟後會直接打開行程；先上傳機票／住宿，確認後就會放進行事曆。</p><button disabled={!masterReady} onClick={startNew}>{hasCreateDraft?"繼續建立草稿":"建立新出差"}</button></div>:<div className="my-trips-list">{trips.map(t=><article className="my-trip-row panel" key={t.id}><div className="trip-date"><b>{t.startsOn.slice(5).replace("-","/")}</b><span>至 {t.endsOn.slice(5).replace("-","/")}</span></div><div className="trip-main"><em>{t.status==="draft"?"規劃中":"進行中"}</em><h2>{t.name}</h2><p>{t.destinations.map(x=>x.cityName).join(" → ")||"地點待確認"}</p></div><div className="trip-progress"><div><span>我的進度</span><b>{t.progress}%</b></div><i><em style={{width:`${t.progress}%`}}/></i><small>{t.remaining?`還有 ${t.remaining} 項待完成`:"已完成"}</small></div><div className="trip-row-actions"><button className="open" onClick={()=>openTrip(t)}>開啟出差</button><details className="trip-more"><summary aria-label="更多操作">⋯</summary><div className="trip-more-popover"><button onClick={()=>editTrip(t)}><span aria-hidden="true"><PencilIcon/></span>編輯</button><button className="delete" onClick={()=>setPendingDelete(t)}><span aria-hidden="true"><TrashIcon/></span>刪除</button></div></details></div></article>)}</div>}
  {pendingDelete&&<div className="trip-delete-overlay" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget&&!deleting)setPendingDelete(null)}}><section className="trip-delete-dialog" role="alertdialog" aria-modal="true" aria-labelledby="delete-trip-title"><span className="trash-mark" aria-hidden="true"><TrashIcon/></span><h2 id="delete-trip-title">刪除這趟出差？</h2><p>「{pendingDelete.name}」的行程與相關資料將移至封存。這個動作不會直接永久清除資料。</p><div><button disabled={deleting} onClick={()=>setPendingDelete(null)}>取消</button><button className="confirm-delete" disabled={deleting} onClick={deleteTrip}>{deleting?"正在刪除…":"刪除出差"}</button></div></section></div>}
 </section>;
 return <>
  <section className="wizard-shell">
   ...
   <div className="wizard-progress three-steps">{["地點、日期與名稱","邀請成員","確認建立"].map((x,i)=><div className={step===i+1?"active":step>i+1?"done":""} key={x}><i>{step>i+1?<CheckIcon/>:i+1}</i><span>{x}</span></div>)}</div>
   <div className="create-card panel wizard-card">{state==="loading"?<div className="wizard-loading">正在載入上次草稿…</div>:<>
    {step===1&&<>
     <div><h2>{mode==="edit"?"編輯出差":"建立新出差"}</h2><p>先選地點，再選日期；系統會自動產生出差名稱。</p><small className="draft-lifecycle-note">公司地點主檔：{masterVersion||"載入中"}</small>{mode==="create"&&<small className="draft-lifecycle-note">建立中會自動保存；離開時會再次確認草稿已寫入伺服器。</small>}</div>
     <div className="merged-destination"><h3>1. 選擇國家與城市</h3><div className="destination-list">{form.stops.map((s,i)=><div className="destination-stop" key={i}><b>目的地 {i+1}</b><button onClick={()=>setForm(v=>({...v,stops:v.stops.filter((_,n)=>n!==i)}))} disabled={form.stops.length===1}>移除</button><label>國家<SearchableSelect key={`country-${i}-${s.country}`} value={s.country} options={countries} placeholder="輸入國名或代碼搜尋" onChange={value=>changeStop(i,"country",value)}/></label><label>城市<SearchableSelect key={`city-${i}-${s.country}-${s.city}`} value={s.city} options={s.country?(destinationOptions[s.country]??[]):[]} placeholder={s.country?"輸入城市搜尋":"請先選國家"} disabled={!s.country} onChange={value=>changeStop(i,"city",value)}/></label></div>)}</div><button className="add-destination" onClick={()=>setForm(v=>({...v,stops:[...v.stops,{country:"",city:""}]}))}><PlusIcon/> 新增另一個國家／城市</button></div>
