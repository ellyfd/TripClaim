...
export default function CreateTripWizardLive({onCreate}:{onCreate:(trip:OpenedTrip)=>void}){
 ...
 if(mode==="list")return <section className="my-trips-page">
  <header className="my-trips-head"><div><span>我的出差</span><h1>全部出差</h1><p>選一趟出差就直接進行程；機票、住宿、待辦與行事曆都在同一頁，報支另外處理。</p></div><div><strong>{trips.length}</strong><small>趟出差</small><button disabled={!masterReady} onClick={startNew}>{hasCreateDraft?"繼續建立草稿":"＋ 建立新出差"}</button></div></header>
