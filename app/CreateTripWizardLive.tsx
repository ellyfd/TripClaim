...
export default function CreateTripWizardLive({onCreate,onExpense}:{onCreate:(trip:OpenedTrip)=>void;onExpense?:(trip:OpenedTrip)=>void}){
 ...
 const openTrip=(trip:Trip,target:"overview"|"expense")=>{const opened={id:trip.id,name:trip.name};if(target==="expense"&&onExpense)onExpense(opened);else onCreate(opened)};
