...
export default function CreateTripWizardLive({onCreate}:{onCreate:(trip:OpenedTrip)=>void}){
 ...
 if(mode==="list")return <section className="my-trips-page">
  <header className="my-trips-head"><div><span>我的出差</span><h1>全部出差</h1><p>選一趟出差先看總覽，再進入行程、行前準備或自己的報支。</p></div><div><strong>{trips.length}</strong><small>趟出差</small><button onClick={startNew}>{hasCreateDraft?"繼續建立草稿":"＋ 建立新出差"}</button></div></header>
