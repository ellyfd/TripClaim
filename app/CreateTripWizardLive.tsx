 ...
 return <>
  <section className="wizard-shell">
   ...
   <div className="create-card panel wizard-card">{state==="loading"?<div className="wizard-loading">正在載入上次草稿…</div>:<>
    {step===1&&<>
     <div><h2>{mode==="edit"?"編輯出差":"建立新出差"}</h2><p>先選地點，再選日期；系統會自動產生出差名稱。</p>{mode!=="edit"&&<button type="button" className="reset-trip" onClick={()=>{setForm(initial);setNameEdited(false);setStep(1);setState("ready")}}>清除舊草稿，建立另一趟</button>}</div>
