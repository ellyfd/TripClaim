...
export default function CreateTripWizardLive({onCreate,onExpense}:{onCreate:(trip:OpenedTrip)=>void;onExpense?:(trip:OpenedTrip)=>void}){
 ...
 useEffect(()=>{if(state==="loading")return;const timer=setTimeout(()=>fetch("/api/drafts",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({flow:"create",step,payload:form})}).catch(()=>null),350);return()=>clearTimeout(timer)},[step,form,state]);
 ...
 if(mode==="list")return <section className="my-trips-page">
  <header className="my-trips-head"><div><span>我的出差</span><h1>全部出差</h1><p>從這裡查看進度、繼續安排行程或處理自己的報帳。</p></div><div><strong>{trips.length}</strong><small>趟出差</small><button onClick={startNew}>＋ 建立新出差</button></div></header>
  {state==="loading"?<div className="trips-empty panel">正在載入我的出差…</div>:trips.length===0?<div className="trips-empty panel"><h2>還沒有出差</h2><p>建立第一趟後，行程、機票、住宿與報帳進度都會集中在這裡。</p><button onClick={startNew}>建立新出差</button></div>:<div className="my-trips-list">{trips.map(t=><article className="my-trip-row panel" key={t.id}><div className="trip-date"><b>{t.startsOn.slice(5).replace("-","/")}</b><span>至 {t.endsOn.slice(5).replace("-","/")}</span></div><div className="trip-main"><em>{t.status==="draft"?"規劃中":"進行中"}</em><h2>{t.name}</h2><p>{t.destinations.map(x=>x.cityName).join(" → ")||"地點待確認"}</p></div><div className="trip-progress"><div><span>我的進度</span><b>{t.progress}%</b></div><i><em style={{width:`${t.progress}%`}}/></i><small>{t.remaining?`還有 ${t.remaining} 項待完成`:"已完成"}</small></div><div className="trip-row-actions"><button className="open" onClick={()=>openTrip(t,"itinerary")}>共同行程</button><button className="claim" onClick={()=>openTrip(t,"expense")}>我的報帳</button><details className="trip-more"><summary aria-label="更多操作">⋯</summary><div className="trip-more-popover"><button onClick={()=>editTrip(t)}><span aria-hidden="true">✎</span>編輯</button><button className="delete" onClick={()=>setPendingDelete(t)}><span aria-hidden="true">🗑︎</span>刪除</button></div></details></div></article>)}</div>}
  {pendingDelete&&<div className="trip-delete-overlay" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget&&!deleting)setPendingDelete(null)}}><section className="trip-delete-dialog" role="alertdialog" aria-modal="true" aria-labelledby="delete-trip-title"><span className="trash-mark" aria-hidden="true">🗑︎</span><h2 id="delete-trip-title">刪除這趟出差？</h2><p>「{pendingDelete.name}」的共同行程與相關資料將移至封存。這個動作不會直接永久清除資料。</p><div><button disabled={deleting} onClick={()=>setPendingDelete(null)}>取消</button><button className="confirm-delete" disabled={deleting} onClick={deleteTrip}>{deleting?"正在刪除…":"刪除出差"}</button></div></section></div>}
 </section>;
 return <>
  <section className="wizard-shell">
   <button className="back-to-trips" onClick={()=>setMode("list")}>← 回我的出差</button>
   ...
   <div className="create-card panel wizard-card">{state==="loading"?<div className="wizard-loading">正在載入上次草稿…</div>:<>
    {step===1&&<>
     <div><span>步驟 1 / 3</span><h2>{mode==="edit"?"編輯出差":"建立新出差"}</h2><p>先選地點，再選日期；系統會自動產生出差名稱。</p>{mode!=="edit"&&<button type="button" className="reset-trip" onClick={()=>{setForm(initial);setNameEdited(false);setStep(1);setState("ready")}}>清除舊草稿，建立另一趟</button>}</div>
