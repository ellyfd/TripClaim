...
export default function MissingRequirements({tripId,refreshKey=0}:{tripId?:string;refreshKey?:number}){
 ...
 return <section className="missing-live panel"><div className="missing-live-head"><div><span>即時完整度</span><h2>{loading?"正在檢查…":missing||review?`還有 ${missing+review} 項要處理`:"目前沒有缺件"}</h2><p>依你本人上傳的文件與付款方式自動計算。</p></div><b>{items.length?Math.round(items.filter(x=>x.status==="complete").length/items.length*100):100}%</b></div>{!loading&&!items.length?<div className="missing-empty">確認文件後，系統會自動列出刷卡單、個人帳單及待確認項目。</div>:<div className="missing-list">{items.map(x=><article key={x.id} className={x.status}><i>{x.status==="complete"?"✓":x.status==="missing"?"!":"?"}</i><div><b>{x.label}</b><span>{x.detail}</span></div><em>{x.status==="complete"?"已齊全":x.status==="missing"?"缺件":"待確認"}</em></article>)}</div>}</section>
}
