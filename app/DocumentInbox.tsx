...
export default function DocumentInbox({tripId,refreshKey=0}:{tripId?:string;refreshKey?:number}){
 const [docs,setDocs]=useState<Doc[]>([]); const [open,setOpen]=useState<string|null>(null); const [loading,setLoading]=useState(true); const [message,setMessage]=useState("");const [undo,setUndo]=useState<{kind:string;id:string}|null>(null);const [query,setQuery]=useState("");const [bundling,setBundling]=useState(false);
 const load=useCallback(()=>fetch(`/api/documents${tripId?`?tripId=${encodeURIComponent(tripId)}`:""}`).then(x=>x.ok?x.json():null).then(x=>{if(x)setDocs(x.documents??[]);setLoading(false)}).catch(()=>setLoading(false)),[tripId]);
 useEffect(()=>{load()},[load,refreshKey]);
 useEffect(()=>{const reload=()=>load();window.addEventListener("tripclaim:data-changed",reload);return()=>window.removeEventListener("tripclaim:data-changed",reload)},[load]);
 const save=async(d:Doc,form:HTMLFormElement)=>{const fd=new FormData(form);const date=String(fd.get("date")||"");const merchant=String(fd.get("merchant")||"");const cardEvidence=d.documentType.includes("信用卡帳單")||d.documentType.includes("刷卡單");const claimType=cardEvidence?undefined:String(fd.get("claimType")||"餐飲");const originalCurrency=String(d.detectedCurrency??d.currency??"TWD").toUpperCase();const originalAmount=Number(d.detectedAmountMinor??d.amountMinor??0)/100;const reportingCurrency=normalizeCurrency(String(fd.get("reportingCurrency")||"TWD"));const reportingAmount=Number(fd.get("reportingAmount")||0);const city=String(fd.get("city")||"未分類");const paymentMethod=String(fd.get("paymentMethod")||"cash");const cardLast4=String(fd.get("cardLast4")||"");const linkedExpenseId=String(fd.get("linkedExpenseId")||"")||null,billedTwd=Number(fd.get("billedTwd")||0);const suggested=cardEvidence?`EVIDENCE-${date.replaceAll("-","")||"待確認"}_${safe(d.documentType)}_${safe(merchant||"信用卡")}${ext(d.originalName)}`:`EXP-${date.replaceAll("-","")||"待確認"}_${safe(city)}_${safe(claimType||"待確認")}_${safe(merchant||"待確認")}_${reportingCurrency}${reportingAmount||0}_${safe(d.documentType)}${ext(d.originalName)}`;const response=await fetch(`/api/documents/${d.id}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({claimType,expenseDate:date,merchant,originalCurrency,originalAmountMinor:Math.round(originalAmount*100),reportingCurrency,reportingAmountMinor:Math.round(reportingAmount*100),paymentMethod,cardLast4,linkedExpenseId,billedTwdMinor:billedTwd?Math.round(billedTwd*100):null,suggestedName:suggested,status:"ready"})});if(response.ok){setMessage(cardEvidence?"付款證明已配對保存，不會重複建立消費":"已保留原始金額，並按公司申報幣別加入我的報支");setOpen(null);load();setTimeout(()=>setMessage(""),2000)}else{const result=await response.json().catch(()=>null);setMessage(result?.message||"請確認申報幣別與金額")}};
 const remove=async(d:Doc)=>{if(!window.confirm(`確定移到垃圾桶「${d.suggestedName||d.originalName}」？連結費用及相關機票／住宿會一起暫時隱藏，可立即復原。`))return;const response=await fetch(`/api/documents/${d.id}`,{method:"DELETE"});if(response.ok){const result=await response.json();setUndo(result.undo??null);setMessage("已移到垃圾桶，原始檔仍安全保留");setOpen(null);load();window.dispatchEvent(new Event("tripclaim:data-changed"))}else{setMessage("刪除失敗，請重新整理後再試");setTimeout(()=>setMessage(""),2200)}};
 const restore=async()=>{if(!undo)return;const response=await fetch(`/api/trash/${undo.kind}/${undo.id}`,{method:"POST"});if(response.ok){setUndo(null);setMessage("已復原文件、費用與相關行程");load();window.dispatchEvent(new Event("tripclaim:data-changed"));setTimeout(()=>setMessage(""),2200)}};
...
{!(d.documentType.includes("信用卡帳單")||d.documentType.includes("刷卡單"))&&<><label>國家／城市<select name="city" defaultValue="" required><option value="" disabled>選擇公司主檔中的國家／城市</option>{managedCities.map(x=><option key={x}>{x}</option>)}</select>
</label>
...
{!(d.documentType.includes("信用卡帳單")||d.documentType.includes("刷卡單"))&&<><div className="document-original-money"><b>原始單據金額</b><span>{d.detectedCurrency??d.currency??"待確認"} {((d.detectedAmountMinor??d.amountMinor??0)/100).toLocaleString()}</span><small>原始辨識資料會保留，不會被申報金額覆蓋。</small></div>
...
</form>}</article>)}</div>}{message&&<div className="document-message">✓ {message}{undo&&<button onClick={restore}>復原</button>}</div>}</section>
}

function CardEvidenceMatcher({document}:{document:Doc}){
 const [matches,setMatches]=useState<Match[]>([]),[ambiguous,setAmbiguous]=useState(false);useEffect(()=>{fetch(`/api/documents/${document.id}/matches`).then(x=>x.ok?x.json():null).then(x=>{setMatches(x?.candidates??[]);setAmbiguous(Boolean(x?.ambiguous))}).catch(()=>{})},[document.id]);
 return <div className="card-match-box"><label>帳單 TWD 實際請款<input name="billedTwd" type="number" min="0" step="0.01" defaultValue={document.billedTwdMinor?document.billedTwdMinor/100:""} placeholder="不含手續費"/></label><label>配對既有費用<select name="linkedExpenseId" defaultValue={document.linkedExpenseId??""}><option value="">尚未配對</option>{matches.map(x=><option value={x.id} key={x.id}>{x.expenseDate}｜{x.merchant}｜{x.originalCurrency} {((x.originalAmountMinor??0)/100).toFixed(2)}｜{x.score}%</option>)}</select></label>{ambiguous&&<p>找到多筆相近交易，請人工選擇；系統不會自行猜測。</p>}{!matches.length&&<small>目前沒有可安全配對的本人費用，先保存帳單，之後補收據仍可再配對。</small>}</div>
}
