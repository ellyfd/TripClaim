...
export async function requireSystemAdmin(user:ChatGPTUser){const db=await ensureUser(user),now=new Date().toISOString(),existing=await db.select().from(systemUsers),current=existing.find(x=>x.email===user.email);if(!existing.length){await db.insert(systemUsers).values({email:user.email,displayName:user.displayName,role:"admin",createdByEmail:user.email,createdAt:now,updatedAt:now});return db}return current?.role==="admin"?db:null}
